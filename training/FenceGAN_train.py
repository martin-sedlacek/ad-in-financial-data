...
def train_epoch(D, G, D_loss, G_loss, train_dl, G_optimizer, D_optimizer, seq_length, latent_dim, DEVICE,
...
def train_financial(seq_length, latent_dim, tscv_dl_list, D, G, D_optim, G_optim, D_loss, G_loss, random_seed,
    ...
    for train_dl, test_dl in tscv_dl_list:
        for epoch in range(num_epochs):
            train_epoch(D, G, D_loss, G_loss, train_dl, G_optim, D_optim, seq_length, latent_dim, DEVICE, epoch=epoch)

        tmp_em = tmp_mv = 0
        for X, Y in test_dl:
            em, mv = emmv(D, X.to(DEVICE), DEVICE=DEVICE)
            tmp_em += em
            tmp_mv += mv
...
def train_kdd99(seq_length, latent_dim, train_dl, test_dl, D, G, D_optim, G_optim, D_loss, G_loss, random_seed, num_epochs, DEVICE):
    set_seed(random_seed)
    start = time.time()
    for epoch in range(num_epochs):
        train_epoch(D, G, D_loss, G_loss, train_dl, G_optim, D_optim, seq_length, latent_dim, DEVICE, epoch=epoch)
